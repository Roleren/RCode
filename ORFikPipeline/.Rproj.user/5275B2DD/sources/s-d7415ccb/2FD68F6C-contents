...
#' @export
heatMap_single_old <- function(region, tx, reads, outdir,
...
# foldchangetest <- function(rfp, rna) {
#   res <- DESeq(rfp)
#   plotMA(results(res))
#   te <- (rfp + 1) / (rna + 1)
#
...
#' @param tx a grl
#' @param reads a GAlignment or GRanges
#' @param median_multiplier a character or numeric, default "0.99", either a quantile if input is string[0-1],
#'  like "0.99", or numeric value if input is numeric. How much bigger than median value is lowest allowed ?
#' @param min_cutoff a character or numeric, default "0.999", either a quantile if input is string[0-1],
...
#' @inheritParams ORFik::heatMap_single
