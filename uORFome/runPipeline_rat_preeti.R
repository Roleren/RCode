...
{ # This part will vary according to what your experiments looks like, here I pick 3 stages to use
  # Load experiments
  df.cage <- NULL
  df.rfp  <- read.experimentl("Preeti_RFP") # RNA-seq is optional, but makes results better
  df.rna  <- read.experimentl("Preeti_RNA")

  orfikDirs(mainPath = "/export/valenfs/projects/Hakon/Preeti_rat/uORFome_rat",
            df.rfp, df.rna, df.cage,
            organism = "Rattus norvegicus") # <- scientific name for organism, will let you know if you misspelled
}
