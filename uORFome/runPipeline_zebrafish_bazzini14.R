...
if (0) { # Do this only once!
  ...
  shiftFootprintsByExperiment(df.rfp, accepted.lengths = 26:30) # pick different read lengths if you want
  heatMapRegion(df.rfp, type = "pshifted", shifting = "5prime")
  # RNA (mRNA-seq)
...
{ # This part will vary according to what your experiments looks like, here I pick 3 stages to use
  ...
  uORFomePipe:::orfikDirs(mainPath = "/export/valenfs/projects/Hakon/uORFome_zebrafish_bazzini",
            df.rfp, df.rna, df.cage, biomart = NULL,
...
preds <- seqFeatures[pred == 1 & StartCodons == "AAG",]
